...
async function init_kick_chat_ws(channel_id, chat_id) {
	const res = await fetch(`https://kick.com/api/v2/channels/${channel_id}/messages`);
	const json = await res.json();
	json.data.messages.reverse().forEach(msg_data => {
		on_kick_message(msg_data);
	});
	live_chat.scrollTop = live_chat.scrollHeight;
	chat_ws = new WebSocket('wss://ws-us2.pusher.com/app/32cbd69e4b950bf97679?protocol=7&client=js&version=8.4.0&flash=false');
	chat_ws.onopen = () => {
	...
	chat_ws.onmessage = (event) => {
		...
		// {"event":"GiftedSubscriptionsEvent","data":"{\"chatroom_id\":64158499,\"gifted_usernames\":[\"leanka\"],\"gifter_username\":\"AlecwelanderSwinkardasTV\",\"gifter_total\":1}","channel":"chatroom_64158499"}
...
async function init_kick_chat(channel_name) {
	...
	await get_seventv('KICK', kick_user_id);
	{
...
function on_twitch_message(channel_name, msg_data) {
	...
	if (command.includes(`PRIVMSG #${channel_name}`)) {
...
async function init_twitch_chat(channel_name) {
	...
	await get_seventv('TWITCH', twitch_user_id);
	{
		...
		for (const message of json.messages) {
			on_twitch_message(channel_name, message);
		}
	}
	chat_ws = new WebSocket('wss://irc-ws.chat.twitch.tv/');
	chat_ws.onopen = () => {
	...
	chat_ws.onmessage = event => {
		on_twitch_message(channel_name, event.data);
	};
}
...
async function set_chat_mode(platform, target, encrypt) {
	seventv = {};
	chat_platform = platform;
	chat_encrypt = encrypt;
	if (chat_ws && ws.readyState !== WebSocket.CLOSED) {
		chat_ws.close();
	}
	elem_set_inner_html(live_chat, '');
	chat_ready = false;
	if (platform === 'twtv') {
		await init_twitch_chat(target);
	} else if (platform === 'kick') {
		await init_kick_chat(target);
	}
	chat_ready = true;
}